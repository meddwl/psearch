...
from psearch.database import init_db, save_model, CompoundRecord
...
def create_argparser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description=(
            'Generates a database of RDKit molecule objects, '
    ...
    parser.add_argument(
        '-i', '--input', metavar='FILENAME', required=True, type=str,
        help='input file of 2D SDF or SMILES format (tab-separated).',
    )
    parser.add_argument(
        '-d', '--db', metavar='FILENAME.db', required=True, type=str,
        help=(
            'output database file. Should have DB extension. '
            'If there is a database with the same name, then the tool will stop.'
        ),
    )
    parser.add_argument(
        '-b', '--bin_step', metavar='NUMERIC', type=int, default=1,
        help=(
            'Bin width (Å) for discretising pharmacophore feature coordinates. '
            'Smaller values increase fingerprint resolution. Default: 1 Å.'
        ),
    )
    parser.add_argument(
        '-s', '--nstereo', metavar='INTEGER', type=int, default=5,
        help=(
            'Maximum number of stereoisomers to generate per compound. '
            'Stereocentres with explicitly specified configurations will not '
            'be altered.'
        ),
    )
    parser.add_argument(
        '-n', '--nconf', metavar='INTEGER', type=int, default=50,
        help='number of generated conformers.',
    )
    parser.add_argument(
        '-e', '--energy_cutoff', metavar='NUMERIC', type=float, default=None,
        help=(
            'conformers with energy difference from the lowest one greater '
            'than the specified threshold will be discarded.'
        ),
    )
    parser.add_argument(
        '-r', '--rms', metavar='NUMERIC', type=float, default=None,
        help=(
            'Only conformers with pairwise RMS higher than this threshold (Å) '
            'will be kept. Default: None (keep all conformers).'
        ),
    )
    parser.add_argument(
        '--seed', metavar='INTEGER', type=int, default=-1,
        help='integer to init random number generator. Default: -1 (means no seed).',
    )
    parser.add_argument(
        '-p', '--pharm_def', metavar='FILENAME', type=str, default=None,
        help=(
    ...
    parser.add_argument(
        '-c', '--ncpu', metavar='INTEGER', type=int, default=1,
...
def check_dupl_input(fname: str) -> tuple[list, list, list, list]:
    """Check the input file for duplicate SMILES or molecule IDs.

    Duplicate SMILES are skipped and flagged; duplicate IDs for distinct SMILES
    are renamed by appending a '#N' suffix. Returns parallel lists used to
    build the database.
    ...
    Returns:
        Tuple of (mols, smis, mol_ids, flags) where flags[i] is True if
        molecule i was a duplicate or was renamed.
    """
    n = 0
    ...
    for mol, mol_name in read_input(fname):
        ...
        if canon_smi in smi_set:
            n += 1
            sys.stdout.write(
                f'WARNING! The molecule with name {mol_name} meets the second '
                f'time in the input file. This molecule will be omitted\n'
            )
            ...
            n += 1
            suffix = 1
            # if more than two molecules share the same name, keep incrementing
            while mol_name in mol_id_set:
                sys.stdout.write(
                    f'WARNING! The molecule ID {mol_name} meets the second '
                    f'time for the distinct molecule SMILES. New molecule ID '
                    f'will be given to this molecule - '
                    f'{mol_name.split("#")[0]}#{suffix}\n'
                )
                mol_name = f"{mol_name.split('#')[0]}#{suffix}"
                suffix += 1
            box_mols.append(mol)
...
def get_mol(mols) -> Generator[tuple[Chem.Mol, str], None, None]:
...
def gen_stereo(mol: Chem.Mol, num_isomers: int) -> tuple[Chem.Mol, ...]:
    ...
    opts = StereoEnumerationOptions(tryEmbedding=False, maxIsomers=num_isomers)
    isomers = tuple(EnumerateStereoisomers(mol, options=opts))
    return isomers


def gen_conf(mol: Chem.Mol, num_confs: int, seed: int) -> Chem.Mol | None:
    ...
    Returns:
        ...
        or None if embedding produced no conformers.
    """
    mol = Chem.AddHs(mol)
...
def remove_confs(mol: Chem.Mol, energy: float | None, rms: float | None) -> None:
    """Remove conformers that fail energy or RMS diversity filters, in-place.

    Conformers with MMFF energy more than `energy` kcal/mol above the
    lowest-energy conformer are removed first. Then, among the surviving
    conformers, redundant ones with pairwise RMS below `rms` Å are removed
    (keeping the lower-energy member of each close pair). Conformers are
    reindexed starting from 0 after pruning.

    Args:
        mol: RDKit Mol with embedded and MMFF-minimised conformers.
        energy: Maximum allowed energy difference (kcal/mol) from the lowest
                conformer. None disables the energy filter.
    ...
    for conf in mol.GetConformers():
        ...
        if ff is None:
            sys.stderr.write(
                Chem.MolToSmiles(mol) + ". MMFFGetMoleculeForceField return NONE\n"
            )
            return
    ...
    if rms is not None:
        rms_list = [
            ...
            for i1, i2 in combinations(kept_ids, 2)
        ]
    ...
    # Reindex conformers starting from 0
    for i, conf in enumerate(mol.GetConformers()):
        conf.SetId(i)


def gen_data(
    mol_cid: tuple[Chem.Mol, str],
    nconf: int,
    nstereo: int,
    energy: float | None,
    rms: float | None,
    ...
    Enumerates up to `nstereo` stereoisomers, embeds up to `nconf` conformers
    per stereoisomer (filtered by `energy` and `rms`), then computes pmapper
    pharmacophore feature coordinates and binary fingerprints for each conformer.
    ...
    for i, mol in enumerate(isomers):
        ...
        if mol is None:
            sys.stderr.write(
                f"{mol_name}: conformer embedding failed, skipping isomer {i}\n"
            )
            continue
        remove_confs(mol, energy, rms)

        phs = utils.load_multi_conf_mol(
            mol, smarts_features=pharm_def, bin_step=bin_step
        )
        mol_dict[i] = mol
...
def create_db(
    ...
    processes each compound in parallel (gen_data) and writes results to the
    psearch shelve database at `out_fname`. If any molecules were renamed or
    skipped, a corrected SMILES file is written alongside the input file.
    ...
    try:
        with Pool(nprocess) as p:
            ...
            for i, data in enumerate(
                p.imap_unordered(worker, get_mol(zip(mols, cids)), chunksize=1),
                1,
            ):
                if not data:
                    continue
                ...
                if verbose and i % 200 == 0:
                    date_time = datetime.now().strftime("%m/%d/%Y, %H:%M:%S")
                    sys.stdout.write(
                        f"{i} number of molecules were processed. {date_time}\n"
                    )

        if sum(flags) > 0:
            sys.stdout.write(
                f"\nWARNING! {sum(flags)} molecules were omitted and/or renamed "
            ...
            while os.path.exists(new_in_fname):
                new_in_fname = (
                    f"{os.path.splitext(in_fname)[0]}-updated{suffix}.smi"
                )
                suffix += 1
            df_cid = pd.DataFrame(
                data={'smi': smis, 'cid': cids, 'if_changed': flags}
            )
            df_cid.to_csv(new_in_fname, sep='\t', index=None)
            sys.stdout.write(
                f"The molecules corresponding to the generated database are "
                f"stored in {new_in_fname} file\n\n"
            )
    finally:
        if verbose:
            date_time = datetime.now().strftime("%m/%d/%Y, %H:%M:%S")
...
def entry_point() -> None:
    ...
    if args.bin_step < 0 or args.nstereo <= 0 or args.nconf <= 0:
        sys.exit(
            "--bin_step, --nstereo, --nconf can not be less 0.\n"
            "--stereo and/or --nconf can not be set to 0, otherwise, "
            "the database will not be created correctly."
        )
    ...
    create_db(
        in_fname=os.path.abspath(args.input),
        out_fname=fdb,
        nconf=args.nconf,
